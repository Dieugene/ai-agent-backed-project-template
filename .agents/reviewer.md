...
**Архитектурные проблемы →** Эскалировать к Architect:
- Нарушение ADR
- Изменение публичных интерфейсов
- Проблемы требующие архитектурного решения

**Проблем нет →** Передать Architect для приемки:
...
[Если есть архитектурные проблемы: Прими архитектурное решение и передай обратно Analyst]
```
...
Задача: Реши архитектурные проблемы из review. Создай/обнови ADR.
...
После проверки:
- **Статус:** Принято / Требует доработки / Архитектурная проблема
- **Путь к отчету:** `01_tasks/NNN_название/review_NN.md`
- **Ключевые проблемы:** 1-2 предложения (если есть)
- **Следующий шаг:** "Используй промпт ниже для [Analyst/Architect]"
</o>
