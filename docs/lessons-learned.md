...
Поставили опциональный watcher (см. [Pool Communication §7.5](pool-communication.md)), который будит сессию при входящей задаче. Хотелось, чтобы он «сам себя перезапускал». Любая попытка вынести перезапуск наружу (отдельный `.bat`, само-spawn через `Start-Process`, перезапуск из хука) — будит мимо: разбуженной сессии нет.
...
- Идемпотентность перевзвода — через приватный леджер (id уже виденных задач), **не** через мутацию статуса задачи в общем store (ломает Tasks-UI, гонка с CLI).
- Near-100% — owner-gated **Stop-хук**, блокирующий завершение хода без живого watcher'а; но он «липче» и трогает общий hook-конфиг — применять осознанно.
...
- **Единственный признак «inbox-watcher жив» — свежий `lock-<owner>.txt`** в `.watcher-state/`, а не «висит какой-то фоновый процесс». При жалобе «watcher не сработал» первым делом проверять свежесть лока.
