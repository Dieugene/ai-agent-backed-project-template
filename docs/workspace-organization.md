...
subagent-driven режиме — планирование, TDD, ревью и верификация идут через
`superpowers`-скилы, а независимые части — через субагентов.
См. [Tech Lead Mode](tech-lead-mode.md).
...
> **Исключение — управляющая (не-агентская) сессия.** Правило выше — про
> рабочие сессии агентов/пулов. Отдельная **управляющая** сессия
> (оркестратор пространства, которая заводит новые пулы и ведёт стандарты)
> может намеренно жить в **под-папке** (`<workspace-root>/.control/` и т.п.),
> а не в корне. Причина — дискаверинг скилов Claude Code идёт **вниз** от
> cwd: из корня в листинг сессии затягиваются описания скилов **всех**
> дочерних пулов, раздувая контекст (listing budget) без пользы для
> управляющей роли. Cwd в собственной под-папке ограничивает каскад её
> скилами. Доступ ко всему пространству при этом цел (абсолютные пути +
> skip-permissions). Механика листинга и бюджета — [The Skills
...
### 1. Заводить пачку ролей впрок

**Симптом:** создаём стопку role-файлов и расписываем между ними handoff'ы —
а потом 90% работы всё равно делает один Tech Lead, потому что роли не
наполняются.
...
- [The Skills System](the-skills-system.md) — механика скилов, listing
  budget, каскад дискаверинга скилов вниз от cwd (см. §7 про cwd
  управляющей сессии).
- [Claude Code Setup](claude-code-setup.md) — среда пространства:
  effort-уровни, статус-лайн, именованные сессии, файловая memory,
  browser-изоляция.
- [Safety Guards](safety-guards.md) — глобальный rm-guard и защитные
  хуки, работающие в том числе под `--dangerously-skip-permissions`.
- [Working Principles](working-principles.md) — стоячие принципы работы
  любого агента пространства.
