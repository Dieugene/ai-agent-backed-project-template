...
**Split-layout пулы.** Doc-артефакты (`_agent_pool_setup-*`, авто-обвязка
`CLAUDE.md`, сниппеты) кладутся в **папку манифеста**, а wrapper и `.warp` —
в `manifest.root`. Для standalone-пула это одна и та же папка; но у
split-layout пула (манифест и `00_docs`/`CLAUDE.md` в проектной подпапке, а
wrapper'ы в общей `scripts/`, заданной как `root`) они расходятся — скрипт
разводит их сам (папка манифеста для доков, `root` для обёрток).
Определяется автоматически из пути `-Manifest`, отдельного параметра не нужно.
