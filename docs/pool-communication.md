...
⚠️ **Что здесь устарело после эксплуатации несколькими ролями на одном мосте к человеку.** Замок
сторожа **чата** (не задач) был один на инстанс — и вторая роль пула молча вытесняла первую при
живых признаках здоровья у обеих; замок разведён по ролям, в него добавлен пульс с порогом
живости, а адресата задаёт ветка чата. Побудка задачей и тихая заметка — разные вещи; тёзки в
разных шинах адресуются парой «шина + роль». Всё это с отказами и числами — [Message Delivery &
Wake-up](message-delivery-and-wakeup.md).
...
- [Message Delivery & Wake-up](message-delivery-and-wakeup.md) — что
  выяснилось, когда пулов и мостов стало несколько: глухая роль, замок на
  роль, ветка как адрес, антифлуд, тёзки, роли на удалённом сервере.
