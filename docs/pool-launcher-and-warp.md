...
Терминальный **пикер пулов** — единая точка запуска: из одного окна выбрать
пул, режим и роли, а дальше пикер сам разворачивает раскладку терминалов
(на Windows — через Warp). Заменяет хождение по папкам за отдельными
wrapper-батниками (`claude-<owner>.bat`) каждого агента.
...
[Board and Watcher](board-and-watcher.md).
...
> Граница ответственности: пикер только **находит** пулы и **зовёт**
> wrapper'ы. Он НЕ взводит вотчер и НЕ несёт стартовый промпт — это делает
> сам wrapper (см. §9 и [Board and Watcher](board-and-watcher.md)).
...
Три уровня выбора, между которыми `Esc` возвращает на шаг назад
(роли → режим → пулы → выход из пикера). UI — `fzf` в ASCII-режиме
...
3. **Роли** (только для «вручную») — отметить нужные.

После выбора пикер **генерит разовый Warp tab-config и открывает окно**
(§7), обновляет MRU и возвращается к списку пулов (можно запустить
следующий). Пикер — **синглтон**: второй запуск не плодит окно, а выводит
уже открытое на передний план (named-mutex + `.state\owner.pid`).
