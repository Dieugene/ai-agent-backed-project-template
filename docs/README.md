...
сессий, координирующихся через Tasks API + общий mailbox + UserPromptSubmit hook.
...
| [Pool Communication](pool-communication.md) | Координационный стандарт pool: env vars, Tasks API, общий mailbox `.inbox/`, UserPromptSubmit hook, топология (inter-project vs intra-project), per-owner handoff. Инвариант top-level `owner` — центральная грабля. Накопление `completed` и инъекция всего списка в контекст → дворник стора. Опциональный push-watcher входящих задач (§7.5). |
...
| [Wrapper and Hook Scripts](wrapper-and-hook-scripts.md) | Полные рабочие примеры скриптов pool-инфры: `pool-launch.ps1` (auto-resume по display name), `inject-inbox.ps1` (UserPromptSubmit hook с полным кодом), wrapper-батник на агента, опциональный push-watcher `wait-for-task.ps1` + `Get-PendingTasks.ps1` (§4.5), обязательный дворник стора задач `archive-completed-tasks.ps1` (§4.6). Discovery walk-up, UTF-8 для кириллицы. |
...
| [Lessons Learned](lessons-learned.md) | 16 антипаттернов и пределов с реальной практики: top-level owner, hook walk-up, размытие границ, helper-launcher по аналогии, mojibake кириллицы, изменения pool-инфры без согласования, перевзвод push-watcher'а как действие агента, коллизия имени «watcher», накопление `completed` раздувает контекст. |
