...
| [Memory Actualization](memory-actualization.md) | Надстройка над памятью: машинный список записей на перепроверку (порция 7–19, иначе 93 % без исхода), метка проверки, помощник-вердиктчик и порядок «свой исход → вердикт», закрытие пачкой с обязательной цитатой, подсадные, три измерителя, которые врали, и честная граница: измерителя «сработало ли знание» нет. |
...
| [Agent Messaging](agent-messaging/README.md) | **Слой осознанных коммуникаций поверх шины** — введение и девять файлов: устройство шины, обязательства (понятия, жизненный цикл, поля), как формулировать «чего жду» и «по какому событию увижу», события реестра (тихое закрытие без письма), реестр и сводка между пулами, обратная связь ролям, эксплуатация, замеры (база «до», два прогона стенда, пороги приёмки) и решения владельца дословно. Мишень — длинные рассуждения о малозначимом, не поклоны (их не оказалось: 1 на 3013 писем). |
| [Message Delivery & Wake-up](message-delivery-and-wakeup.md) | Когда пулов и мостов несколько: глухая роль как главный отказ; замок сторожа на роль, а не на инстанс (4 случая глухоты за месяц → 0 за восемь дней; выборка небольшая); пульс с порогом живости; ветка чата как адрес; задача против тихой заметки; тёзки и ключ «шина + роль»; метки «только ответ» и обслуживания; антифлуд по слову адресата; роли на удалённом сервере и сироты после обрыва ssh. |
...
### Доступ к пулу издалека
| Документ | О чём |
|----------|-------|
| [Web Console](../web-console/README.md) | Живые сессии пула в браузере: терминал за реверс-прокси, страница с панелями, обмен файлами с ролями по адресату. Почему готовые туннели отпали (география, вендор, память), десяток решений, которые дорого выяснять заново (Origin за прокси, размер окна, сигнал при обрыве, вставка из буфера, перетаскивание над рамкой), и семь способов, которыми **проба** врала на исправной системе. |
| [remote-bridge](../remote-bridge/README.md) | Удалённый пульт к своей сессии через Telegram: текст и голос, работает за NAT, одна копия кода на весь workspace. |
