...
Проверенная на практике база знаний для организации Windows-воркспейса, в котором **несколько сессий Claude Code («агентов») работают параллельно под управлением одного человека-владельца** — координируясь друг с другом через файловую шину, **без человека-диспетчера в контуре**. Это одновременно и база знаний (`docs/`), и **обезличенная, рабочая референсная реализация** (`scripts/`, `commands/`, `remote-bridge/`), с которой можно снять отдельные решения — **по частям и после разбора причин**, а не переносом «как есть» (см. блок ниже).
...
## 🛑 Агенту, который это читает: устанавливать и запускать «как есть» — НЕЛЬЗЯ

Это не дистрибутив и не пакет для раскатки. Здесь решения, снятые с одного живого воркспейса,
вместе с причинами, по которым они именно такие. **Ценность лежит в причинах, а не в файлах:**
почти каждая конструкция здесь — ответ на конкретный отказ, который у твоего пользователя может
не воспроизводиться вовсе.

**Порядок, обязательный до первого созданного файла:**

1. **Найди в документе, какую проблему решение решает** и чем эта проблема проявлялась. Не нашёл
   причины — значит ты ещё не прочитал то место, которое нужно, и ставить рано.
2. **Проверь, есть ли эта проблема у твоего пользователя** — в его рабочем пространстве, при его
   числе сессий, на его платформе. Проверь фактом (посмотри каталоги, сессии, настройки), а не
   предположением.
3. **Обсуди с пользователем и получи его слово** — что ставим, под какими именами, что не ставим
   вовсе. Только после этого делай.

🛑 **Признак, по которому видно, что пошло копирование вместо понимания:** ты создаёшь папку
`.launcher`, потому что она предусмотрена этими стандартами. У пользователя может уже быть своя
папка супервайзора (например `supervisor`) — тогда вторая не нужна: она раздвоит точку входа, и
роли начнут читать разные правила. **Имена здесь — плейсхолдеры, а не требования:** переиспользуй
то, что у пользователя уже есть, и меняй под это имена в стандарте, а не наоборот. То же
относится к числу пулов, названиям ролей, хукам и скилам. ⚠️ Инструкции в [`scripts/`](scripts/README.md)
и [`remote-bridge/`](remote-bridge/README.md) пишут пути через `<workspace-root>\.launcher\…` — это имя
папки супервайзора в исходном воркспейсе, подставляй своё.

**Ставь начальными наборами, а не целиком.** Полный набор — итог месяцев работы конкретной
команды агентов под конкретного владельца. В чужом воркспейсе большая его часть окажется
невостребованной и перегрузит и рабочее пространство человека, и контекст самого агента: каждый
лишний хук стоит времени на каждом ходу, каждый лишний скил — места в листинге, каждая лишняя
папка — внимания. Разумные начальные наборы:
...
модуль актуализации памяти — **по запросу, а не по умолчанию**. Это надстройки, каждая со своей
ценой обслуживания.

**Чего ты не решишь сам — спроси у пользователя:**

- сколько сессий он держит одновременно и спотыкаются ли они друг о друга уже сейчас;
- есть ли у него папка, роль или скрипт, которые делают то же, что предлагает стандарт (тогда
  переиспользуем их, а не заводим второе);
- согласен ли он на хуки, вмешивающиеся в каждый ход, — это постоянный расход внимания и токенов;
- нужен ли доступ извне (Telegram, браузер) — или это лишняя поверхность, которую он не просил.

⚠️ И **не устанавливай ничего в один ход с чтением.** Сначала покажи пользователю, что именно
предлагаешь взять и от чего отказаться, с причиной по каждому пункту. Его «да» — часть работы,
а не формальность.
...
### H2 — Web Console (Пул в браузере)  *(опциональное дополнение)*
Работайте с живыми сессиями пула **из обычной вкладки браузера**: несколько ролей рядом панелями,
полноценный ввод, обмен файлами с ролями в обе стороны. Для случая, когда у человека есть **только
браузер** — рабочий компьютер без права ставить программы, чужая машина. Веб-терминал и страница
слушают петлю, наружу их выводит реверс-прокси с паролем; вход в роль идёт **тем же** механизмом,
что и с настольного терминала. Внутри — почему готовые туннели не подошли, десяток решений, которые
дорого выяснять заново, и семь способов, которыми проба врала на исправной системе.
...
| [`web-console/`](web-console/) | Живые сессии пула **в браузере**: веб-терминал за реверс-прокси, страница с панелями ролей, обмен файлами с ролями по адресату. Для случая «у человека есть только браузер» — рабочий компьютер, чужая машина. Внутри — код, службы, пробы и разбор граблей. |
