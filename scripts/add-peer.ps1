...
# Канон параметров запуска для НОВОГО peer'а. Донором обычно оказывается лид (-CopyFrom по
# умолчанию = -After = lead), поэтому его -Effort нельзя наследовать: новый peer лидом не
# становится (ниже из манифеста снимается "lead": true). Модель пиним всем — иначе сессия при
# --resume поедет на своей старой, а глобальный /model протечёт в пул.
