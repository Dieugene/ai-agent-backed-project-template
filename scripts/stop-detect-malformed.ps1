...
if ($SelfTest) {
  ...
  $rec = [ordered]@{ ts='2026-07-13T00:00:00'; owner='tech-lead-x'; pool='D:\p\.bus'; session='sid'; count=1; recovered=$true; snippet=$r7.Snippet; transcript=$t7 }
  ...
  Chk ($parsed.recovered -eq $true -and $parsed.owner -eq 'tech-lead-x') 'T11 log: fields round-trip'
